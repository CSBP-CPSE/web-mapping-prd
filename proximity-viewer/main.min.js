(function () {
      ...
      value: function HTMLize(json, fields, na) {
        ...
        fields.forEach(function (f) {
          ...
          if (json.hasOwnProperty(f.id) || f.polish) {
            ...
            if (f.fixed) value = value.toFixed(f.fixed);else if (f.lookup) {
              var look = f.lookup[value];
              var value = look == undefined ? value : look[Core.locale];
            }
          }

          html += "<div><span>".concat(label, " : </span><span>").concat(value, "</span></div>");
        });
  ...
  function (_Evented) {
    ...
    function Map(options) {
      ...
      _this.map.on('click', _this.click);
    ...
    _createClass(Map, [{
      ...
      value: function Choropleth(layers, property, legend, opacity) {
        ...
        if (Array.isArray(opacity) && Array.isArray(legend) && legend.length > 1) {
          legend.forEach(function (l, index) {
            var color = l.color.length == 3 ? "rgba(".concat(l.color.join(','), ",").concat(opacity[index], ")") : "rgba(".concat(l.color.join(','), ")");
      ...
      value: function ChoroplethVarOpac(layers, property, legend, opacity) {
        ...
        layers.forEach(function (l) {
          ...
          _this4.map.setPaintProperty(l, property, classes);
        });
      }
    }, {
      key: "ReorderLayers",
  ...
  function (_Templated) {
    ...
    function Table(container, options) {
      ...
      _this.summary = options.summary; // For last column, replace by label. Super ugly.
    ...
    _createClass(Table, [{
      key: "Template",
      value: function Template() {
        return "<div class='table-widget'>" + "<h2 handle='title'>nls(Table_Title_Temp)</h2>" + "<div id='prx-table' handle='message' class='table-message'>nls(Table_Message)</div>" + "<div handle='table' class='table-container hidden'>" + "<summary>nls(Table_Summary)</summary>" + "<table>" + "<thead>" + "<tr>" + "<th>nls(Table_Field_DBUID)</th>" + "<th>nls(Table_Field_empl.idx)</th>" + "<th>nls(Table_Field_pharm.idx)</th>" + "<th>nls(Table_Field_child.idx)</th>" + "<th>nls(Table_Field_health.idx)</th>" + "<th>nls(Table_Field_groc.idx)</th>" + "<th>nls(Table_Field_edupri.idx)</th>" + "<th>nls(Table_Field_edusec.idx)</th>" + "<th>nls(Table_Field_lib.idx)</th>" + "<th>nls(Table_Field_parks.idx)</th>" + "<th>nls(Table_Field_trans.idx)</th>" + "<th>nls(Table_Field_close)</th>" + "</tr>" + "</thead>" + "<tbody handle='body'></tbody>" + "</table>" + "<div class='navigation'>" + "<button handle='prev' title='nls(Table_Previous_Button)' disabled><img src='assets/arrow-left.png'></button>" + "<span handle='current' class='current'></span>" + "<button handle='next' title='nls(Table_Next_Button)' disabled><img src='assets/arrow-right.png'></button>" + "</div>" + "</div>" + "</div>";
      }
    }, {
      key: "GetDataFileUrl",
      ...
      value: function Populate(item, data) {
        ...
        data.forEach(function (rData) {
          ...
          rData.forEach(function (cData, i) {
            var value = cData;
            if (i == 11) value = _this2.field.lookup[value][Core.locale];else if (i == 0) value = "".concat(value.substr(0, 2), " ").concat(value.substr(2, 2), " ").concat(value.substr(4, 4), " ").concat(value.substr(8, 3));
            Dom.Create("td", {
              innerHTML: value,
      ...
      */
      ...
      value: function UpdateTable(item, page) {
        ...
        this.current.max = this.summary[item.id] || 1; // Get CSV file for selected DB. Extension is json because of weird server configuration. Content is csv.		

        var file = "data/".concat(this.current.item.id, "_").concat(this.current.page, ".json");
        var url = this.GetDataFileUrl(file);
        return Net.Request(url).then(function (ev) {
  ...
  function (_Templated) {
    ...
    function ProxApp(node, config) {
      ...
      _this = _possibleConstructorReturn(this, _getPrototypeOf(ProxApp).call(this, node));
      _this.config = config;
    ...
    _createClass(ProxApp, [{
      key: "Template",
      value: function Template() {
        return "<div class='search-container'>" + "<span class='wb-inv'>nls(Inv_Search_Instructions)</span>" + "<label class='search-label'>nls(App_Search_Label)" + "<div handle='search' class='search'></div>" + "</label>" + "<div class='inv-container'>" + "<a href='#prx-table' class='wb-inv wb-show-onfocus wb-sl'>nls(Inv_Skip_Link)</a>" + "</div>" + "</div>" + "<div class='instructions'>nls(App_Instructions)</div>" + "<div class='map-container'>" + "<div handle='map' class='map'></div>" + "</div>" + "<div class='table-container'>" + "<div handle='table' class='table'></div>" + "</div>" + "<div class='link-symbols-container'>" + "<a href='nls(STS_Link)' class='link-symbols' target='_blank' title='nls(STS_Title)'>nls(STS_Label)</a>" + "</div>";
      }
    }, {
      key: "AddMap",
      ...
      value: function AddSearch() {
        ...
        this.config.search.items = this.config.search.items.map(function (i) {
          return {
            id: i[0],
            name: i[1],
            label: "".concat(i[1], " (").concat(_this2.FormatCSD(i[0]), ")"),
      ...
      value: function OnMapClick_Handler(ev) {
        ...
        if (!db || !csd) return;
        var item = null;
        this.config.search.items.forEach(function (i) {
          if (i.id == csd.properties.uid) item = i;
        });
        if (!item) return; // ie11 doesn't support find
        ...
        db.properties.CSDUID = "".concat(csd.properties.name, " (").concat(db.properties.CSDUID, ")");
      ...
      key: "FormatDB",
      value: function FormatDB(dbuid) {
        var csd = this.FormatCSD(dbuid);
        return "".concat(csd).concat(dbuid.substr(7, 1), " ").concat(dbuid.substr(8, 3));
      } // CSD 35 06 008 and DB 35 06 0700 029

    }, {
      key: "FormatCSD",
      value: function FormatCSD(csduid) {
        return "".concat(csduid.substr(0, 2), " ").concat(csduid.substr(2, 2), " ").concat(csduid.substr(4, 3));
      }
    }]);
  ...
  function Start(results) {
    ...
    Promise.all([p1, p2, p3, p4]).then(function (results) {
      ...
      var app = new ProxApp(node, config);
    });
  }
